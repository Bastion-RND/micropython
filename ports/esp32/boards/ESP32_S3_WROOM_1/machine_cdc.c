    ...
    switch (event->type)
    {
        case CDC_EVENT_RX:
            for (;;) {
                ...
                ESP_LOGD(LOG_TAG_CDC, "RX event, received %u bytes", len);
            }
            break;

        case CDC_EVENT_RX_WANTED_CHAR:
            ESP_LOGD(LOG_TAG_CDC, "Received wanted char '%c'",
                event->rx_wanted_char_data.wanted_char);
            break;

        case CDC_EVENT_LINE_STATE_CHANGED:
            ESP_LOGD(LOG_TAG_CDC, "Line state changed [DTR: %i, RTS: %i]",
                event->line_state_changed_data.dtr,
                event->line_state_changed_data.rts);
            break;

        case CDC_EVENT_LINE_CODING_CHANGED:
            ESP_LOGD(LOG_TAG_CDC, "Line coding changed [bit_rate: %lu, data_bits: %i, parity: %i, stop_bits:%i]",
        ...
        default:
            ESP_LOGE(LOG_TAG_CDC, "cdcacm_callback - unknown event: %i", event->type);
            break;
    }
}
...
void tud_cdc_tx_complete_cb(uint8_t itf) {
    ...
    ESP_LOGD(LOG_TAG_CDC, "TX complete, remained %lu",
...
STATIC mp_obj_t machine_cdc_deinit(mp_obj_t self_in) {
    machine_cdc_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ESP_LOGD(LOG_TAG_CDC, "deinit self: %p, buf ptr: %p", self, self->rx_buf.buf);
    if (self) {
        memset(self->rx_buf.buf, 0x00, self->rx_buf.size);
        self->rx_buf.iget = 0;
        self->rx_buf.iput = 0;
    }
    ESP_LOGD(LOG_TAG_CDC, "deinited...");
...
STATIC mp_obj_t machine_cdc_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    if (esp_log_level_get(LOG_TAG_CDC) != LOG_LEVEL_MACHINE_CDC) {
        esp_log_level_set(LOG_TAG_CDC, LOG_LEVEL_MACHINE_CDC);
    }
...
STATIC mp_obj_t machine_cdc_connected(mp_obj_t self_in) {
    ...
    ESP_LOGD(LOG_TAG_CDC, "connected: %s", connected ? "yes" : "no");
...
STATIC mp_obj_t machine_cdc_line_state(mp_obj_t self_in) {
    ...
    ESP_LOGD(LOG_TAG_CDC, "line state: DTR(%i), RTS(%i)", (line_state >> 0) & 0x1, (line_state >> 1) & 0x1);
    return dict;
}
...
STATIC mp_obj_t machine_cdc_line_coding(mp_obj_t self_in) {
    ...
    ESP_LOGD(LOG_TAG_CDC, "line coding: bit_rate=%lu, data_bits=%i, parity=%i, stop_bits=%i",
...
//    machine_cdc_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ...
    ESP_LOGD(LOG_TAG_CDC, "written to queue: %u bytes", bytes_written);
